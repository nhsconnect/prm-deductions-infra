...
describe('rotateApiKeys', () => {
    ...
    it('should rotate api key when the parameter is tagged with RotateApiKey', async () => {
        ...
        expect(getRotateApiKeyTag).toHaveBeenCalledWith('/repo/env/api-keys/api-key-user/user');
        expect(getRotateApiKeyTag).toHaveBeenCalledWith('/repo/env/api-keys/api-key-user/user-1');
        ...
        expect(rotateApiKey).toHaveBeenCalledWith('/repo/env/api-keys/api-key-user/user');
        expect(rotateApiKey).not.toHaveBeenCalledWith('/repo/env/api-keys/api-key-user/user-1');
        expect(rotateApiKey).toHaveBeenCalledTimes(1)
        expect(removeRotateApiKeyTag).toHaveBeenCalledWith('/repo/env/api-keys/api-key-user/user');

        expect(restartServices).toHaveBeenCalledWith(['/repo/env/api-keys/api-key-user/user'])
    ...
    it('should not rotate api key when the parameter is not tagged with RotateApiKey', async () => {
        getParamsByPath.mockResolvedValueOnce(['/repo/env/api-keys/api-key-user/user', '/repo/env/api-keys/api-key-user/user-1']);

        await rotateApiKeys(false);

        expect(getParamsByPath).toHaveBeenCalledWith('/repo/env/user-input/api-keys/');
        expect(rotateApiKey).not.toHaveBeenCalledWith('/repo/env/api-keys/api-key-user/user-1')
        expect(removeRotateApiKeyTag).not.toHaveBeenCalledWith('/repo/env/api-keys/api-key-user/user-1');
        expect(restartServices).not.toHaveBeenCalledWith(['/repo/env/api-keys/api-key-user/user-1'])
    });
